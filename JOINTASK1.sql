...
SELECT FIRST_NAME, LAST_NAME, employee.DEPARTMENT_ID,DEPARTMENT_NAME
FROM employee
JOIN departments
...
--Q3.  find the first name, last name, salary, 
--and job grades for all employees.

SELECT FIRST_NAME,LAST_NAME,SALARY,GRADE_LEVEL
FROM employee
JOIN Job_grades
ON  employee.salary >= Job_grades.LOWEST_SAL AND employee.salary <= Job_grades.HIGHEST_SAL

-- Q4.  find all those employees who work in department ID 90. 
...
--Q5 find those employees whose first name contains the letter ‘z’. Return first name, last name, department, city, and state province.
SELECT FIRST_NAME, LAST_NAME,DEPARTMENT_NAME,CITY,STATE_PROVINCE 
FROM employee
JOIN departments
ON employee.department_id = departments.DEPARTMENT_ID
...
--Q6  find all departments, including those without employees. Return first name, last name, department ID, department name.  
SELECT FIRST_NAME, LAST_NAME,EMPLOYEE.DEPARTMENT_ID,DEPARTMENT_NAME
FROM employee
LEFT JOIN departments
ON employee.department_id = departments.DEPARTMENT_ID

--Q7  find the employees who earn less than the employee of ID 182. Return first name, last name and salary

SELECT FIRST_NAME, LAST_NAME, SALARY
FROM employee
WHERE SALARY < (SELECT SALARY FROM employee WHERE EMPLOYEE_ID = 103);




-- Q8 find the employees and their managers. Return the first name of the employee and manager
SELECT E.FIRST_NAME, M.FIRST_NAME AS Manager
from employee E
JOIN employee M
ON E.employee_id= m.manager_id

--Q9 display the department name, city, and state province for each department
...
--Q11 find the employees and their managers. Those managers do not work under any manager also appear in the list. 
...
ON E.employee_id = m.manager_id 
 ...
 --Q12 find the employees who work in the same department as the employee with the last name Taylor. 
 --Return first name, last name and department ID.
 SELECT first_name, last_name, departments.department_id
FROM employee 
JOIN departments
ON employee.department_id = departments.department_id
WHERE departments.DEPARTMENT_ID IN (SELECT department_id FROM employee WHERE last_name = 'Taylor')



--Q13 calculate the average salary, the number of employees receiving commissions in that department.
--Return department name, average salary and number of employees.


SELECT d.department_name, AVG(e.salary) AS avg_salary, COUNT(e.employee_id) AS num_employees_with_commission
FROM employee e
JOIN departments d 
...
--Q14 find the department name and the full name (first and last name) of the employee.
...
--Q15 find the department name, full name (first and last name) of the employee and their city.
...
--Q16 find full name (first and last name), and salary of all employees working in any department in the city of London
...
--Q17 find the department name, department ID, and number of employees in each department.

SELECT department_name, COUNT(employee_id) AS num_OF_employees,DEPARTMENTS.DEPARTMENT_ID
FROM employee 
JOIN departments  
ON employee.department_id = departments.DEPARTMENT_ID
